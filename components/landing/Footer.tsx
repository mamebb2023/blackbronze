...
const Footer = () => {
    ...
    return (
        <footer className="relative bg-[#0B0B0B] text-white overflow-hidden">
            ...
            {/* Top gradient border */}
            <div className="absolute top-0 left-0 right-0 h-px bg-linear-to-r from-transparent via-zinc-500/60 to-transparent" />

            {/* Ambient glow */}
            <div className="absolute -top-32 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-zinc-500/10 blur-[100px] rounded-full pointer-events-none" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-16 pb-16 border-b border-white/5">
                        ...
                        <div className="flex items-center gap-3 mt-2">
                            {socialLinks.map(({ icon: Icon, label, href }) => (
                                <motion.a
                                    ...
                                    className="w-9 h-9 rounded-full border border-white/10 bg-white/5 flex-center text-zinc-400 hover:text-white hover:border-white/40 transition-colors"
                                >
                        ...
                        <ul className="flex flex-col gap-3">
                            {navLinks.map(({ label, href }) => (
                                <li key={label}>
                                    ...
                                    >
                                        <span className="w-4 h-px bg-zinc-700 group-hover:w-6 group-hover:bg-zinc-400 transition-all duration-300" />
                                        {label}
                                    </Link>
                                </li>
                            ))}
                    ...
                    <div>
                        ...
                        <motion.a
                            ...
                            whileHover={{ x: 3 }}
                            transition={{ duration: 0.2 }}
                            className="inline-flex items-center gap-2 text-sm font-medium text-zinc-300 hover:text-white border border-zinc-500/40 hover:border-white/20 rounded-full px-5 py-2.5 transition-colors"
                        >
                            Let&apos;s talk
